...
def get_trending_repositories(top_size):
    ...
    request_parametres = {'q': 'created:>={date_week_ago}'.format(date_week_ago=str(date_week_ago)),
                          'sorted': 'stars'}
...
if __name__ == '__main__':
    ...
    print("Today is: {today_date}".format(today_date=str(datetime.date.today())))
    ...
    for number, repository in enumerate(trending_repositories_created_for_a_week, 1):
        print(number,
              ...
              'stars: {stars} | '.format(stars=str(repository['stargazers_count'])),
              'Open issues: {open_issues}'.format(open_issues=str(repository['open_issues']))
              )
